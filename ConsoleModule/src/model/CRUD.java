...
public class CRUD extends Conections {
	...
	public static void deleteUser(int id,String user) throws SQLException, ClassNotFoundException {
		connect();
		String Table="";
		if(user.equals("sellers")) {
			for(int i=1;i<=3;i++) {
				switch(i) {
				case 1:{Table="sellers";break;}
				case 2:{Table="land";break;}
				case 3:{Table="house";break;}
				}
				String del_query="DELETE FROM "+Table+" WHERE sellersID =?;";
				pst=con.prepareStatement(del_query);
				pst.setInt(1, id);
				pst.executeUpdate();
			}	
		}
			...
			if(user.equals("buyers")) {
				Table="buyers";
				String del_query="DELETE FROM "+Table+" WHERE buyersID =?;";
				pst=con.prepareStatement(del_query);
				pst.setInt(1, id);
				pst.executeUpdate();
			}
